...
grep -Fq 'resolve_pmc_pdf' <<<"$smoke_output" || fail "MCP smoke output is missing resolve_pmc_pdf"
...
grep -Fq 'doiTestProcess.stdin.write(`${target}\n`)' "$PROJECT_ROOT/app/research-console/server.js" || fail "Console DOI test does not reuse the existing browser process"
! grep -Fq 'tell application "Terminal"' "$PROJECT_ROOT/app/research-console/server.js" || fail "Console DOI test still launches a new Terminal window"
...
grep -Fq 'CLOAKBROWSER_DOWNLOAD_MODE: downloadMode' "$PROJECT_ROOT/app/research-console/server.js" || fail "Console DOI test does not select a download policy"
...
grep -Fq '重用同一個 cloakbrowser_research session 與同一個頁籤' "$PROJECT_ROOT/app/research-console/public/index.html" || fail "Console prompt does not require same-tab DOI validation"
grep -Fq 'resolve_pmc_pdf' "$PROJECT_ROOT/app/research-console/public/index.html" || fail "Console prompt is missing current PMC Cloud resolution"
...
grep -Fq 'Reuse one `cloakbrowser_research` session and the same page/tab' "$PROJECT_ROOT/codex/skills/ars-literature-workflow/SKILL.md" || fail "ARS skill does not require same-tab DOI validation"
grep -Fq '`cloakbrowser_research.resolve_pmc_pdf`' "$PROJECT_ROOT/codex/skills/ars-literature-workflow/SKILL.md" || fail "ARS skill is missing current PMC Cloud resolution"
